...
from . models import Especialidades, DadosMedico, is_medico, DatasAbertas, Consulta, Documento
...
def consulta_area_medico(request, id_consulta):
    ...
    if request.method == "GET":
        ...
        context = {'consulta': consulta,'is_medico': is_medico(request.user), 'documentos': documentos}
...
def finalizar_consulta(request, id_consulta):
    ...
    if request.user != consulta.data_aberta.user:
        messages.add_message(request, constants.ERRO, 'Esta Consulta não é sua.')
        return redirect(f'/medicos/abrir_horario')

    consulta.status == "F"
    consulta.save()
    messages.add_message(request, constants.SUCCESS, 'Consulta finalizada com sucesso.')
    return redirect(f'/medicos/consulta_area_medico/{id_consulta}')
